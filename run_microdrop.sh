...
# --- Argument Parsing ---
DEVICE="dropbot" # Default device
STASH=0          # Default to false

# Loop through all passed arguments
while [[ "$#" -gt 0 ]]; do
    case $1 in
        --device|-d)
            # Convert input to lowercase for safer matching
            DEVICE="${2,,}"
            shift 2
            ;;
        --stash|-s)
...
if [[ "$DEVICE" != "dropbot" && "$DEVICE" != "opendrop" ]]; then
...
ENV_ROOT="$PARENT_PATH/.pixi/envs/default" # Fixed path: removed redundant /microdrop-py/
...
if [ -d "$TARGET_PATH" ]; then
    ...
    echo -e "${CYAN}Checking for src updates on 'main' branch...${NC}"
    pixi run git checkout main
    pixi run git pull
    echo -e "${CYAN}----------------------------------------${NC}"

    # Use a case statement to route the launch command
    case "$DEVICE" in
        opendrop)
